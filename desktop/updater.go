...
const (
	...
	manifestFallback    = "https://github.com/esengine/Rexion/releases/latest/download/latest.json"
	defaultDownloadPage = "https://github.com/esengine/Rexion/releases/latest"
...
func applyLinux(targz []byte) error {
	bin, err := extractBinary(targz, "Rexion-desktop")
	if err != nil {
		return err
	}
...
func applyWindows(installer []byte) error {
	f, err := os.CreateTemp("", "Rexion-update-*.exe")
	if err != nil {
		return err
	}
	name := f.Name()
