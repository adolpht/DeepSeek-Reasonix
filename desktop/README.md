# Rexion Desktop (Wails shell)

A native desktop window around the Rexion Go kernel. The same
...
`desktop/` is its own Go module (`module Rexion/desktop`, `replace Rexion =>
...
`Rexion/` so it can still import the `Rexion/internal/*` kernel.
...
- **macOS** — open `Rexion-darwin-universal.dmg` and drag Rexion into
  ...
  ```sh
  xattr -dr com.apple.quarantine /Applications/Rexion.app
  ```
