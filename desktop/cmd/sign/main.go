...
// by substring (file names embed the key, e.g. Rexion-darwin-arm64.zip), so the
...
// (Rexion.key) and the public key (Rexion.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "Rexion.key")
	pubPath := filepath.Join(dir, "Rexion.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "Rexion desktop release")
		out := f + ".minisig"
