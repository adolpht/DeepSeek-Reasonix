...
// todoItem represents a single todo entry in the Rexion database.
...
// dbPath returns the path to the Rexion SQLite database. It checks
// REXION_HOME first, then falls back to ~/.rexion/.
func dbPath() string {
	if home := os.Getenv("REXION_HOME"); home != "" {
		return filepath.Join(home, "data.db")
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return filepath.Join(".", ".rexion", "data.db")
	}
	return filepath.Join(home, ".rexion", "data.db")
}

// openDB opens (and initialises) the Rexion SQLite database. It creates the
