// Command reasonix-plugin-mail is an MCP stdio server exposing email tools
// (read, send, search, classify) to Reasonix. It is a standalone binary
...
func main() {
	log.SetPrefix("reasonix-plugin-mail: ")
	log.SetFlags(0)
...
func handleLine(line []byte, w *bufio.Writer) error {
	...
	case "initialize":
		resp.Result = map[string]any{
			...
			"serverInfo":      map[string]any{"name": "reasonix-plugin-mail", "version": version},
		}
	case "tools/list":
