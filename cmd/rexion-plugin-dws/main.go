// Command Rexion-plugin-dws is an MCP stdio server that wraps the DingTalk
...
// tools so the Rexion agent can operate DingTalk via natural language.
//
...
//
// Rexion surfaces its tools as mcp__dws__dws_call / dws_schema / dws_auth / dws_check.
//
...
func main() {
	log.SetPrefix("Rexion-plugin-dws: ")
	log.SetFlags(0)
...
func handleLine(line []byte, w *bufio.Writer) error {
	...
	case "initialize":
		resp.Result = map[string]any{
			...
			"serverInfo":      map[string]any{"name": "Rexion-plugin-dws", "version": version},
		}
	case "tools/list":
...
// dwsCheckTool is the auto-start tool. When the plugin connects, Rexion
	...
	run: func(args map[string]any) (any, error) {
		...
		if !dwsBinExists() {
			msg := "dws CLI is not installed or not found on PATH.\n\n" +
				...
				"After installing, restart Rexion or reconnect the dws plugin."
