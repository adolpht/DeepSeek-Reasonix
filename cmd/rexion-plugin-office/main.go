// Command Rexion-plugin-office is an MCP stdio server exposing document
// (docx/markdown/pdf) read/write/convert/template tools to Rexion.
//
...
//
// Rexion then surfaces tools as mcp__office__read_docx / write_docx /
...
func main() {
	log.SetPrefix("Rexion-plugin-office: ")
	log.SetFlags(0)
...
// --- JSON-RPC framing (mirrors Rexion-plugin-example / -sheet) ---
...
func handleLine(line []byte, w *bufio.Writer) error {
	...
	case "initialize":
		resp.Result = map[string]any{
			...
			"serverInfo":     map[string]any{"name": "Rexion-plugin-office", "version": version},
		}
	case "tools/list":
