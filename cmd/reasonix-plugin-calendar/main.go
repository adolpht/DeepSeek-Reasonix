// Command reasonix-plugin-calendar is an MCP stdio server exposing calendar
// event and todo management tools to Reasonix. It is a standalone binary wired
// in via reasonix.toml:
//
//	[[plugins]]
//	name    = "calendar"
//	command = "reasonix-plugin-calendar"
//
// Reasonix then surfaces its tools as mcp__calendar__read_event / add_event /
...
//
// Todo storage uses a SQLite database at $REASONIX_HOME/data.db (default
// ~/.reasonix/data.db).
//
...
func main() {
	log.SetPrefix("reasonix-plugin-calendar: ")
	log.SetFlags(0)
...
// --- JSON-RPC framing (mirrors reasonix-plugin-office / -search) ---
...
func handleLine(line []byte, w *bufio.Writer) error {
	...
	case "initialize":
		resp.Result = map[string]any{
			...
			"serverInfo":      map[string]any{"name": "reasonix-plugin-calendar", "version": version},
		}
	case "tools/list":
...
var listTodoTool = toolDef{
	name:        "list_todo",
	description: "List todo items from the Reasonix todo database",
	readOnly:    true,
	schema: map[string]any{
		"type": "object",
		"properties": map[string]any{
...
var addTodoTool = toolDef{
	name:        "add_todo",
	description: "Add a new todo item to the Reasonix todo list",
	readOnly:    false,
	schema: map[string]any{
		"type": "object",
		"properties": map[string]any{
