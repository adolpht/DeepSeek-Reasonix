// Command reasonix-plugin-sheet is an MCP stdio server exposing spreadsheet
// (xlsx/csv) read/write/query/chart tools to Reasonix. It is a standalone binary
...
//
// Reasonix then surfaces its tools as mcp__sheet__read_sheet / write_sheet /
...
// cmd/reasonix-plugin-example for the contract). Logs go to stderr; stdout is
...
func main() {
	log.SetPrefix("reasonix-plugin-sheet: ")
	log.SetFlags(0)
...
// --- JSON-RPC framing (mirrors reasonix-plugin-example) ---
...
func handleLine(line []byte, w *bufio.Writer) error {
	...
	case "initialize":
		resp.Result = map[string]any{
			...
			"serverInfo": map[string]any{"name": "reasonix-plugin-sheet", "version": version},
		}
	case "tools/list":
