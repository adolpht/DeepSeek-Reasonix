...
An MCP stdio server that gives [Reasonix](../../README.md) spreadsheet (xlsx/csv) capabilities: read, write, query (SQL-like), and chart.

Part of the [Personal Agent roadmap](../../docs/personal-agent-roadmap.md) — Phase 1. Zero侵入: it is a standalone binary wired in via `reasonix.toml`; the Reasonix core never imports it.
...
Reasonix then surfaces the four tools under the `mcp__sheet__` namespace.
...
```sh
go build ./cmd/reasonix-plugin-sheet/     # → reasonix-plugin-sheet(.exe)
```

Pure Go, `CGO_ENABLED=0`, single binary — same distribution story as Reasonix itself.
...
```sh
go test ./cmd/reasonix-plugin-sheet/ -v
```
