// Command Rexion-plugin-slides is an MCP stdio server exposing PowerPoint
// (pptx) creation/editing/theming/export tools to Rexion.
//
...
func main() {
	log.SetPrefix("Rexion-plugin-slides: ")
	log.SetFlags(0)
	probePython()
...
func handleLine(line []byte, w *bufio.Writer) error {
	...
	case "initialize":
		resp.Result = map[string]any{
			...
			"serverInfo":      map[string]any{"name": "Rexion-plugin-slides", "version": version},
		}
	case "tools/list":
