...
#   macOS:   Rexion-darwin-<arch>.zip                  (ditto archive; updater channel)
#            Rexion-darwin-universal.dmg               (drag-to-install; human download)
#   Windows: Rexion-windows-<arch>-installer.exe       (NSIS per-user installer; updater channel)
#            Rexion-windows-<arch>.zip                 (portable human download)
#   Linux:   Rexion-linux-<arch>.tar.gz                (bare binary)
#
...
APPNAME="Rexion"            # wails.json productName -> Rexion.app
BINNAME="Rexion-desktop"    # wails.json outputfilename -> linux binary name
...
case "$os" in
darwin)
	# Wails names the bundle after outputfilename (Rexion-desktop.app); repackage
	# it as Rexion.app for a clean user-facing name. Ad-hoc sign the copy (still
	...
	cp -R "build/bin/Rexion-desktop.app" "$app"
