...
<p align="center">
  ...
  &nbsp;·&nbsp;
  <a href="https://esengine.github.io/DeepSeek-Rexion/">官方网站</a>
  &nbsp;·&nbsp;
  <strong><a href="https://discord.gg/XF78rEME2D">Discord</a></strong>
</p>

> [!IMPORTANT]
> **Rexion 1.0 是用 Go 从零重写的版本** —— 本分支(`main-v2`)已是新的默认分支,后续开发都在这里。
> 早期的 `0.x` TypeScript 版本转为 **legacy**,保留在 [`v1`](https://github.com/esengine/DeepSeek-Rexion/tree/v1) 分支(仅维护)。
> 详见**[迁移指南](./docs/MIGRATING.md)**。`npm i -g Rexion` 仍是安装命令——`1.0.0`+ 装的是 Go 二进制,`0.x` 是 legacy TS 版。
...
```sh
npm i -g Rexion                  # 任意系统;自动拉取对应平台的原生二进制
...
Rexion chat                       # 然后在会话里运行 /init 生成 AGENTS.md（项目记忆）
...
优先级：**flag > `./Rexion.toml` > `~/.config/Rexion/config.toml` > 内置默认值**。
...
`mode`）。`Rexion chat` 会在 writer 调用前征求同意（`1` 本次 · `2` 本会话 · `3` 拒绝，兼容 `y/a/n`）；
`Rexion run` 保持自主运行但仍然遵守 `deny`。完整 schema 与契约见
...
Rexion 是一个 MCP 客户端。`[[plugins]]` 的 `type` 选择传输：`stdio`（默认）启动本地子进
...
**已有 Claude Code 的 `.mcp.json`？** 直接放到项目根目录，Rexion 会原样读取——其
`mcpServers` 规范（`command`/`args`/`env`、`type`/`url`/`headers`、`${VAR}` 展开）
与 `[[plugins]]` 字段一一对应。两处来源会合并加载；同名时以 `Rexion.toml` 为准。
...
**从 `0.x` 升级？** 旧的 `~/.Rexion/config.json` 仍会被读取(读其 `mcpServers`、并遵从
`mcpDisabled`),作为最低优先级来源——所以 MCP 服务器照常可用;方便时再把它们挪进
`Rexion.toml` 的 `[[plugins]]` 或 `.mcp.json`。

### 斜杠命令

`Rexion chat` 里,内置命令(`/compact`、`/new`、`/rewind`、`/tree`、`/branch`、`/switch`、`/todo`、`/model`、`/effort`、`/mcp`、`/help`)在本地执行。
`/tree` 查看已保存的对话分支,`/branch [name]` 从当前对话末端分支,`/branch <turn> [name]`
从较早的 checkpoint 轮次分支,`/switch <id|name>` 切换到另一个分支。**自定义命令**
是放在 `.Rexion/commands/`(项目)或 `~/.config/Rexion/commands/`(用户)下的 Markdown 文件——
...
在消息里写 `@` 引用,Rexion 会在发送前解析成带标签的上下文块:`@path/to/file`(或
...
`Rexion setup` 刻意保持首次体验极简：选 provider → 输入 key（所选 provider 的所有
SKU 都会启用）。若要让两个模型协同（执行器 + 规划器，各自独立、缓存稳定的
session），向导后手动在 `Rexion.toml` 加一行即可：
...
Planner 会看到已加载的 `Rexion.md` / `AGENTS.md` 记忆，并拿到一小组只读研究工具，
...
如果 Rexion 帮你省了时间或 token，欢迎请杯咖啡。捐助不会换来 feature 优先级，也不会影响 issue 的处理顺序——就是「谢谢」。
...
下面这些朋友的工作塑造了 Rexion 今天的样子 —— 综合 commit 数和代码量两个维度。
...
<p align="center">
  <a href="https://github.com/esengine/DeepSeek-Rexion/graphs/contributors">
    <img src="https://contrib.rocks/image?repo=esengine/DeepSeek-Rexion&max=100&columns=12" alt="esengine/DeepSeek-Rexion 贡献者" width="860"/>
  </a>
</p>
...
<p align="center">
  ...
  <br/>
  <sub>由 <a href="https://github.com/esengine/DeepSeek-Rexion/graphs/contributors">esengine/DeepSeek-Rexion</a> 社区共建</sub>
</p>
