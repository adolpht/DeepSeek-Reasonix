...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `Rexion chat`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold Rexion.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `Rexion chat`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `Rexion setup`.",
	StepSetKeyHint: "run `Rexion setup`, or export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "Rexion help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `Rexion chat`",
	...
	OutputStyleHint:    "set agent.output_style in Rexion.toml to apply one (takes effect next session)",
	ThemeHeader:        "themes:",
	...
	SlashMCPNone:                 "no MCP servers configured — add a [[plugins]] entry in Rexion.toml",
	...
	ArgLanguageAuto: "auto-detect from Rexion_LANG / locale",
	...
	ListHooksNone:       "hooks: none active — configure in .rexion/settings.json (project, after trust) or ~/.rexion/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in Rexion.toml ([[plugins]]) or a project .mcp.json",

	MemoryNone:             "memory: none — add with “/remember <note>” or create Rexion.md in the project root",
	...
	MissingKeyIntro:       "Rexion.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (run `Rexion setup` or export DEEPSEEK_API_KEY=...), then run `Rexion run \"your task\"`.",
	...
	SkipStaleCustomEntryFmt:    "skipping stale %q entry from Rexion.toml (pointing at %s) — please remove it from [[providers]]",
	...
	UsageRunHint:              "usage: Rexion run [--model NAME] <task>",
	ErrorPrefix:               "error:",
	...
	ProviderErrAuth:                "Authentication failed (HTTP 401): your API key is missing, wrong, or expired. Check the key in .env or run `Rexion setup`.",
	...
	UsageBody: `Rexion — a config- and plugin-driven coding agent (multi-model)

Usage:
  Rexion chat [--model NAME] [-c|--continue] [--resume]   interactive session (multi-turn; -c resumes the latest, --resume picks one)
  Rexion run  [--model NAME] [--max-steps N] <task>   run one task and exit
  Rexion serve [--model NAME] [--addr HOST:PORT]      serve the session over HTTP+SSE (browser client at /)
  Rexion acp [--model NAME]                           serve Agent Client Protocol over stdio (also: Rexion --acp)
  Rexion setup [path]                                 interactive config wizard; writes Rexion.toml (+ .env)
  Rexion config auto-plan [off|on]                    configure automatic plan mode
  Rexion mcp <add|remove|list>                        manage MCP servers in Rexion.toml
  Rexion doctor [--json]                              print redacted local diagnostics
  Rexion version
  Rexion help

Examples:
  Rexion chat
  Rexion chat --continue
...
Configuration:
  Resolution: flag > ./Rexion.toml > ~/.config/Rexion/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'Rexion setup' to scaffold a config; see docs/SPEC.md.
`,
