...
// $LC_ALL / $REXION_LANG. Unknown locales return "" so DetectLanguage falls
...
// TestDetectLanguagePriority verifies override beats env and that REXION_LANG
// beats LANG. With a clean env we fall back to English.
func TestDetectLanguagePriority(t *testing.T) {
	t.Setenv("REXION_LANG", "")
	t.Setenv("LC_ALL", "")
	...
	t.Setenv("REXION_LANG", "en")
	if got := DetectLanguage(""); got != "en" {
		t.Errorf("REXION_LANG=en overriding LANG=zh: got %q, want en", got)
	}
