...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — Rexion setup
	StepSetKey      string // step 2 command label

	// `Rexion init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // Rexion chat step desc
	StepRunDesc    string // Rexion run step desc
	HelpFooter     string // dim footer linking to Rexion help
	...
	WroteFileFmt          string // "Wrote %s" — used for Rexion.toml and .env both
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from Rexion.toml (pointing at %s) — please remove it"
	...
	UsageRunHint              string // "usage: Rexion run [--model NAME] <task>"
...
//
// Priority: override > REXION_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"REXION_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
