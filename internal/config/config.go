// Package config loads Rexion's runtime configuration from TOML. Resolution order:
// flag > project ./Rexion.toml > user ~/.config/Rexion/config.toml > built-in defaults.
...
// Config is Rexion's runtime configuration.
type Config struct {
	ConfigVersion    int                    `toml:"config_version"`
	DefaultModel     string                 `toml:"default_model"`
	Language         string                 `toml:"language"` // ui/model language tag (e.g. "zh"); empty = auto-detect from $LANG / $REXION_LANG
	UI               UIConfig               `toml:"ui"`
...
// lets Rexion fetch the CodeGraph runtime into its cache when CodeGraph is
// enabled but missing; set false to require an explicit `Rexion codegraph
...
//
// The download mirror is controlled via the REXION_CODEGRAPH_MIRROR environment
// variable (default: "https://ghproxy.net" for mainland China accessibility).
// Set to "" to download directly from GitHub, or to another mirror prefix
// (e.g. "https://gh-proxy.com"). Mirror failures fall back to GitHub automatically.
...
// files under ~/.rexion/memory/ (people.md, projects.md, preferences.md,
...
// the project roots (.rexion/.agents/.agent/.claude under the workspace) and
...
type AgentConfig struct {
	...
	// .rexion/output-styles/<name>.md). Empty = the unmodified prompt.
...
type ProviderEntry struct {
	...
	CompletionBudget int               `toml:"completion_budget"` // tokens reserved for model output; subtracted from context_window for compaction thresholds
...
// Rexion.toml's [[plugins]] or a project-root .mcp.json (see loadMCPJSON).
...
const DefaultSystemPrompt = `You are Rexion, a coding agent focused on executing code tasks.
...
func Default() *Config {
	return &Config{
		...
		// Mode "ask" with no rules keeps `Rexion run` autonomous (no TTY → ask
		// resolves to allow) while `Rexion chat` prompts before writers. Users add
		...
		// ~/.rexion/memory/ fold into the system prompt as durable context.
		// EnsureMemoryDir scaffolds them on first boot. Set [pkm] enabled = false
		// to opt out.
		PKM:     PKMConfig{Enabled: true},
		Network: NetworkConfig{ProxyMode: netclient.ModeAuto},
		Providers: []ProviderEntry{
			{Name: "deepseek-flash", Kind: "openai", BaseURL: "https://api.deepseek.com", Model: "deepseek-v4-flash", APIKeyEnv: "DEEPSEEK_API_KEY", BalanceURL: "https://api.deepseek.com/user/balance", ContextWindow: 202_752, Price: &provider.Pricing{CacheHit: 0.02, Input: 1, Output: 2, Currency: "¥"}},
			{Name: "deepseek-pro", Kind: "openai", BaseURL: "https://api.deepseek.com", Model: "deepseek-v4-pro", APIKeyEnv: "DEEPSEEK_API_KEY", BalanceURL: "https://api.deepseek.com/user/balance", ContextWindow: 202_752, Price: &provider.Pricing{CacheHit: 0.025, Input: 3, Output: 6, Currency: "¥"}},
...
// the v0.x ~/.rexion/config.json's mcpServers. A .env in the working directory
...
// each project's Rexion.toml + .env + .mcp.json are resolved independently
...
func LoadForRoot(root string) (*Config, error) {
	...
	projectTOML := "Rexion.toml"
	if root != "." {
		projectTOML = filepath.Join(root, "Rexion.toml")
	}
	...
	// project Rexion.toml doesn't drop the global config's MCP servers.
	...
	// Rexion.toml wins on a name collision (see mergeMCPJSON).
	mcpFile := mcpJSONFile
	if root != "." {
	...
	// Lowest priority: the v0.x ~/.rexion/config.json's mcpServers, so upgrading
...
// LoadForEdit returns a config to seed the `Rexion setup` wizard when reconfiguring:
...
func userConfigPath() string {
	...
	return filepath.Join(dir, "Rexion", "config.toml")
}

// UserConfigPath is the user-global config file (~/.config/Rexion/config.toml),
// or "" when the user config dir can't be resolved.
func UserConfigPath() string { return userConfigPath() }

// UserCredentialsPath is the Rexion-owned global secrets file, beside
// config.toml in the user config dir (e.g. ~/.config/Rexion/credentials). It
...
func UserCredentialsPath() string {
	...
	return filepath.Join(dir, "Rexion", "credentials")
}
...
func ArchiveDir() string {
	...
	return filepath.Join(dir, "Rexion", "archive")
}

// SessionDir is where chat sessions are persisted (one .jsonl per session).
// Used by `Rexion chat --continue` / `--resume` to find the recent ones. Empty
...
func SessionDir() string {
	...
	return filepath.Join(dir, "Rexion", "sessions")
}
...
// existing dirs (UserConfigDir/Rexion/...) so the whole Rexion state tree
...
// MemoryUserDir returns the Rexion user config root (…/Rexion), under which
// the user-global Rexion.md and the per-project auto-memory store live. Empty
...
// commands), in canonical-first order. .rexion is ours; .agents / .agent /
...
// .rexion/settings.json (see internal/hook).
var ConventionDirs = []string{".rexion", ".agents", ".agent", ".claude"}

// conventionSubdirsAsc joins sub under each ConventionDir of base, in ascending
// priority (reverse of ConventionDirs) so the canonical .rexion ends up the
...
// on a name clash. Order: home-dir convention dirs (~/.claude/commands … ~/.rexion/commands),
// the legacy XDG user dir (~/.config/Rexion/commands), then the project's
// convention dirs (.claude/commands … .rexion/commands). Scanning the .claude /
...
func CommandDirsForRoot(root string) []string {
	...
	if dir, err := os.UserConfigDir(); err == nil {
		dirs = append(dirs, filepath.Join(dir, "Rexion", "commands")) // legacy XDG user dir
	}
