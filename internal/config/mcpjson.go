...
// mcpJSONFile is the project-root file Claude Code calls .mcp.json. Rexion reads
...
// legacyConfigPath is the v0.x (TypeScript line) config file, ~/.rexion/config.json.
func legacyConfigPath() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".rexion", "config.json")
}

// loadLegacyMCP reads the v0.x ~/.rexion/config.json and returns its enabled
...
// already declare. Rexion.toml's [[plugins]] win on a name collision: it is the
// Rexion-specific, more explicit of the two, so it overrides the shared,
