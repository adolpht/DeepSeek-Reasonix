...
func TestDocPathPrefersExisting(t *testing.T) {
	proj := t.TempDir()
	// An existing Rexion.md should keep receiving notes (no split to AGENTS.md).
	if err := os.WriteFile(filepath.Join(proj, "Rexion.md"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	set := Load(Options{CWD: proj})
	if got := set.DocPath(ScopeProject); filepath.Base(got) != "Rexion.md" {
		t.Errorf("should append to the existing Rexion.md, got %s", got)
	}
