// Package memory implements Rexion's persistent memory. It mirrors Claude
// Code's two-layer model while honoring Rexion's cache-first architecture:
//
//   - Hierarchical doc memory: Rexion.md / AGENTS.md files discovered from the
...
const (
	ScopeUser     Scope = "user"     // ~/.config/Rexion/Rexion.md
	ScopeAncestor Scope = "ancestor" // a Rexion.md above the project root
	ScopeProject  Scope = "project"  // ./Rexion.md (committed, shared)
	ScopeLocal    Scope = "local"    // ./Rexion.local.md (personal, git-ignored)
	ScopePKM      Scope = "pkm"      // ~/.rexion/memory/*.md (personal knowledge base)
)

// docNames are the recognized memory filenames at each level, in load order.
// Rexion.md is ours; AGENTS.md and CLAUDE.md are the cross-tool conventions.
...
var docNames = []string{"Rexion.md", "AGENTS.md", "CLAUDE.md"}

// localNames are the personal, git-ignored overrides, highest precedence.
var localNames = []string{"Rexion.local.md", "AGENTS.local.md", "CLAUDE.local.md"}
