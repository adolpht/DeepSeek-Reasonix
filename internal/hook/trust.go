...
// Trust gates project hooks. A project's .rexion/settings.json can run
...
// root. The trust flag lives in user-global state (~/.rexion/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.rexion/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.rexion.
...
// TrustPath is ~/.rexion/trust.json (homeDir overrides ~).
