...
// (.rexion/settings.json, only when the project is trusted) and a global
// (~/.rexion/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = ".rexion"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is ~/.rexion/settings.json (homeDir overrides ~).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(home(homeDir), SettingsDirname, SettingsFilename)
}

// ProjectSettingsPath is <root>/.rexion/settings.json.
