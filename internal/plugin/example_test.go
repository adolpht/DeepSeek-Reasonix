...
// buildExamplePlugin compiles cmd/Rexion-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "Rexion-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "rexion/cmd/Rexion-plugin-example").CombinedOutput()
	if err != nil {
