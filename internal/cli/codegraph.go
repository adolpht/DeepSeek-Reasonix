...
// codegraphCommand backs `Rexion codegraph` — managing the CodeGraph
// code-intelligence runtime that Rexion otherwise fetches lazily on first use.
...
func codegraphStatus() int {
	...
	if p, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
		...
		fmt.Printf("%-13s %s\n", "resolved:", "(not installed — run `Rexion codegraph install`)")
	}
	return 0
}

func codegraphUsage() {
	fmt.Print(`Rexion codegraph — manage the CodeGraph code-intelligence runtime

Usage:
  Rexion codegraph install   download + cache the runtime for this platform
  Rexion codegraph status    show config, cache dir, and resolved launcher
