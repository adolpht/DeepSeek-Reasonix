...
func BuiltinSubagentTools(store *Store, runner SubagentRunner, profileResolver ...ProfileResolver) []tool.Tool {
	...
	specs := []struct {
		...
		{"contract_draft", "contract-draft",
			"Draft a professional contract (service/purchase/NDA/employment/custom) in an isolated subagent — uses built-in write_docx tool to generate DOCX with placeholder fields for unfilled terms. No external plugin required.",
			"Describe the contract to draft: type (service/purchase/nda/employment/custom), parties, subject matter. E.g. '起草一份办公用品采购合同' or 'service contract between Acme Corp and Beta LLC for IT consulting'."},
		{"weekly_report", "weekly-report",
			"Generate a structured weekly report (周报) from git commit history in an isolated subagent — categorizes commits, summarizes progress, and outputs a DOCX using built-in write_docx tool. No external plugin required.",
			"Optional focus or team filter. The subagent reads git log for the past 7 days. E.g. 'focus on backend team' or leave empty for the full report."},
		{"meeting_minutes", "meeting-minutes",
			"Convert meeting transcripts into structured meeting minutes (会议纪要) in an isolated subagent — extracts topics, decisions, action items, and outputs a DOCX using built-in write_docx tool. No external plugin required.",
			"Meeting transcript text or file path to read. The subagent parses and structures it into minutes with topics, decisions, and action items."},
	}
	var out []tool.Tool
	for _, s := range specs {
