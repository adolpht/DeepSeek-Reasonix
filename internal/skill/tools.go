...
func BuiltinSubagentTools(store *Store, runner SubagentRunner, profileResolver ...ProfileResolver) []tool.Tool {
	...
	specs := []struct {
		...
		{"contract_draft", "contract-draft",
			"Draft a professional contract (service/purchase/NDA/employment/custom) in an isolated subagent — renders template clauses via the office plugin and generates a DOCX with placeholder fields for unfilled terms. Requires the office MCP plugin.",
		...
		{"weekly_report", "weekly-report",
			"Generate a structured weekly report (周报) from git commit history in an isolated subagent — categorizes commits, summarizes progress, and outputs a DOCX. Requires the office MCP plugin.",
		...
		{"meeting_minutes", "meeting-minutes",
			"Convert meeting transcripts into structured meeting minutes (会议纪要) in an isolated subagent — extracts topics, decisions, action items, and outputs a DOCX. Requires the office MCP plugin.",
