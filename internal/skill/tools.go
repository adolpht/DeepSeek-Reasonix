...
func BuiltinSubagentTools(store *Store, runner SubagentRunner, profileResolver ...ProfileResolver) []tool.Tool {
	...
	specs := []struct {
		...
		{"analyze_project", "analyze-project",
			"Analyze a legacy project and generate a comprehensive documentation suite in an isolated subagent — produces architecture docs, data models, API references, module guides, deployment docs as a structured content/ directory with Mermaid diagrams and source citations. Use for onboarding to an unfamiliar codebase.",
			"Absolute or relative path to the project to analyze. The subagent scans the codebase, identifies architecture and modules, then writes documentation files to <project>/content/."},
		{"doc_reviewer", "doc-reviewer",
			"Review and fix documents generated by analyze-project in an isolated subagent. Cross-references every claim against actual source code, fixes inaccurate pseudocode/wrong citations/missing diagrams, and completes cross-references between documents. Use after analyze-project to validate output quality.",
			"Absolute or relative path to the project whose content/ directory to review. The subagent reads every document and cross-references claims against source code."},
		// MiniMax office skills (embedded builtins, replacing old office skills)
		{"minimax_docx", "minimax-docx",
			"Create, edit, or format professional DOCX documents in an isolated subagent — OpenXML SDK pipelines for creation, editing, and template application with XSD validation.",
			"Describe the document task: create a new document, edit an existing one, or apply a template. E.g. create a project report or edit contract.docx to add clause 5."},
		{"minimax_xlsx", "minimax-xlsx",
			"Create, read, analyze, edit, or validate Excel spreadsheets in an isolated subagent — XML template creation, pandas analysis, zero-format-loss editing, formula validation.",
			"Describe the spreadsheet task. E.g. create a budget sheet or analyze sales_data.xlsx for trends."},
		{"minimax_pdf", "minimax-pdf",
			"Generate, fill, or reformat professional PDF documents in an isolated subagent — 15 cover styles, token-based design system, print-ready output.",
			"Describe the PDF task: create from scratch, fill form fields, or reformat. E.g. generate a product intro PDF or fill form.pdf with name=John."},
		{"pptx_generator", "pptx-generator",
			"Generate, edit, or read PowerPoint presentations in an isolated subagent — PptxGenJS creation, XML editing, complete design system with color palettes and style recipes.",
			"Describe the presentation task. E.g. create a 10-slide product launch PPT or edit deck.pptx slide 3."},
		{"ppt_create", "ppt-create",
			"生成精美 PowerPoint 演示文稿——培训课件、产品介绍、汇报演示等场景，自动选配色/风格/版式，输出高质量 PPTX。Runs as a subagent.",
			"描述 PPT 任务：主题、受众、页数、风格偏好。例如：生成一份 10 页的产品培训 PPT，科技风格。"},
		// Tier-1 skills (high-value capabilities adapted from OpenAI skill catalog)
		{"define_goal", "define-goal",
			"Define and clarify a vague intention into a concrete, measurable goal in an isolated subagent — specifies outcome, verification method, scope boundaries, and stop condition. Use when the user gives a fuzzy task and you need to sharpen it before acting.",
			"Describe the project or feature to define goals for. The subagent reads the codebase for context and returns a structured goal specification."},
		{"security_threat_model", "security-threat-model",
			"Generate a repository-grounded threat model in an isolated subagent — enumerates trust boundaries, assets, attacker capabilities, abuse paths, and mitigations. Produces a Markdown threat model report. Use for AppSec-grade security analysis of a codebase.",
			"Describe the codebase path or system to threat-model. The subagent scans the repo, identifies components and boundaries, then writes a threat model Markdown file."},
		{"security_best_practices", "security-best-practices",
			"Audit code against language/framework security best practices in an isolated subagent — supports Python, JavaScript/TypeScript, and Go. Produces a prioritized vulnerability report with fixes. Use for security reviews beyond diff-level analysis.",
			"Describe the code or system to audit for security best practices. The subagent identifies languages/frameworks, applies relevant security guidance, and writes a report."},
		{"screenshot", "screenshot",
			"Take desktop screenshots in an isolated subagent — full screen, window, or pixel region, cross-platform (macOS/Linux/Windows). Uses OS-native commands, no extra dependencies. Use for UI debugging, visual verification, or documentation.",
			"Describe what to screenshot: full screen, app name, window, or pixel region (x,y,w,h). The subagent detects the OS and runs the appropriate capture command."},
		{"playwright", "playwright",
			"Automate browser interactions via Playwright CLI in an isolated subagent — navigation, form filling, snapshots, screenshots, data extraction, UI-flow debugging. CLI-first; no test specs. Use for web testing, scraping, or browser-based workflows.",
			"Describe the browser automation task: URL to open, actions to perform, data to extract. The subagent drives a real browser and returns results."},
	}
	var out []tool.Tool
	for _, s := range specs {
