...
// REXION_CODEGRAPH_E2E so the normal `go test ./...` skips it (no network, no
...
//
//	REXION_CODEGRAPH_E2E=1 REXION_CODEGRAPH_BIN=/path/to/codegraph \
//	  go test ./internal/codegraph/ -run E2E -v -count=1
//
// With REXION_CODEGRAPH_BIN unset it falls back to Resolve("") (bundle / PATH).
func TestE2ECodegraphMCP(t *testing.T) {
	if os.Getenv("REXION_CODEGRAPH_E2E") == "" {
		t.Skip("set REXION_CODEGRAPH_E2E=1 to run the CodeGraph MCP end-to-end test")
	}
	bin := os.Getenv("REXION_CODEGRAPH_BIN")
	if bin == "" {
		var ok bool
		if bin, ok = Resolve(""); !ok {
			t.Fatal("REXION_CODEGRAPH_E2E is set but no codegraph binary found — set REXION_CODEGRAPH_BIN to the launcher path")
		}
	}
