# Rexion Engineering Spec

> Rexion is a coding agent: a thin harness driving multiple models, with **all
...
```
Rexion/
├── go.mod / go.sum          # module Rexion; require BurntSushi/toml
├── Makefile                 # build / cross / vet / fmt / test
├── README.md / README.zh-CN.md
├── Rexion.example.toml         # sample config
├── docs/SPEC.md             # this file
├── cmd/Rexion/main.go          # entry; blank-imports built-in providers/tools
├── cmd/Rexion-plugin-example/  # reference MCP stdio plugin (a runnable example)
└── internal/
    ...
    ├── command/             # custom slash commands loaded from .Rexion/commands/*.md
...
- `cmd/Rexion-plugin-example` is a runnable reference stdio server (`echo`,
...
Long tasks eventually fill the model's context window. Rexion manages this with
...
- Compaction summarizes the older middle of the session into a single briefing —
  ...
  summarized away.
- The dropped originals are archived to `~/.config/Rexion/archive/<timestamp>.jsonl`
...
- **Resolving `Ask`.** The interactive front-end (the chat TUI) prompts the user
  ...
  (`Rexion run`, a sub-agent, anything with no TTY / no approver) cannot prompt, so
...
Out of the box (`mode = "ask"`, no rules) `Rexion run` behaves exactly as before
(writers resolve `Ask`→allow with no TTY), while `Rexion chat` now prompts before
...
- **Custom commands** are Markdown files under `.Rexion/commands/` (project) and
  `~/.config/Rexion/commands/` (user); the project dir overrides the user dir on a
...
- Frontmatter is an optional `---`-fenced block of simple `key: value` lines;
  `description` and `argument-hint` are recognised (no YAML dependency — Rexion
...
Resolution order: **flag > project `./Rexion.toml` > user `~/.config/Rexion/config.toml`
...
# language    = "zh"                # ui language tag; empty = auto-detect from $LANG / $REXION_LANG

[agent]
system_prompt = "You are Rexion, a coding agent..."  # or system_prompt_file = "..."
max_steps     = 25
temperature   = 0.0
...
`Rexion setup` writes this default config so the CLI is usable out of the box.
...
`[[plugins]]`; on a name collision `Rexion.toml` wins (it is the more explicit,
Rexion-specific source). This lets a server already configured for Claude work in
Rexion unchanged.
...
- Build: `CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=$(VERSION)" -o Rexion ./cmd/Rexion`
...
- Sandbox Phase 1: an OS-level jail for `bash` so commands — not just the
  ...
  command unconfined via the permission gate (in `Rexion run`, the command just
...
- "Always allow" persistence writing learned rules back to project config; a
  per-session permission override flag for `Rexion run`.
